...
router.post('/updateOne', function(req, res) {
  islogged.islogged(req, function(err, logged) {
    if (logged.role != "admin") {
      ...
      let obj = req.body;
      req.body.options.forEach((k, i) => {
        console.log(k,typeof(k),obj.answer,typeof(obj.answer))
        let tmp = {
          id: i,
          path: null,
          value: k
      ...
      // check answer == option's value
      let index = opt.findIndex((obj => parseInt(obj.value) === parseInt(req.body.answer)))
      if (index == -1) {
        return res.status(500).send({
          status: "error",
          error: "No option within specified options"
      ...
      qstnController.updateOne(obj, function(err, questions) {
        if (err)
          res.json({
            "status": 500,
            "message": "Internal server error",
    ...
    if (err) {
      ...
      csv()
        ...
        .on('json', (jsonObj) => {
          ...
          let ind = jsonObj.options.indexOf(jsonObj.answer);
          if(ind == -1){
            return res.status(500).send({
              "status":"error",
              "error":jsonObj
          ...
          if (req.body.subject !== 'nepali') {
            ...
            jsonObj.qstn = jsonObj.question;
            jsonObj.question = {};
            jsonObj.question.value = encodeURI(jsonObj.qstn);
            jsonObj.question.path = null;
            jsonObj.opts = jsonObj.options
            jsonObj.options=[]
            jsonObj.opts.forEach(function(key, index) {
              let tmp = {
                value: encodeURI(key),
                id: index,
                path:null
              }
              jsonObj.options.push(tmp);
            })
            jsonObj.answer = ind
            // console.log(jsonObj)
            arr.push(jsonObj);
            ...
            if (jsonObj.answer.length > 1) {
              ...
              jsonObj.qstn = jsonObj.question;
              jsonObj.question = {};
              jsonObj.question.value = encodeURI(preeti.convert_to_unicode(jsonObj.qstn));
              jsonObj.question.path = null;
              jsonObj.opts = jsonObj.options
              jsonObj.options= []
              jsonObj.opts.forEach(function(key, index) {
